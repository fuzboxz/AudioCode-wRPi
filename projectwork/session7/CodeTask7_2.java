...
public class CodeTask7_2 implements BeadsChecker.BeadsCheckable {
    ...
    public void task(AudioContext ac, StringBuffer buf, Object... objects) {
        ...
        String[] msplit = inputData.split("\n");
        Object[] messObj = new Object[msplit.length*2 - 1];
        int counter = 0;
        for (int i = 0; i < msplit.length; i++) {
            //buf.append(msplit[i].split("=")[0]);
            if ((msplit[i].split("=")[0]).equals("Waveform")){
                messObj[counter++] = msplit[i].split("=")[1];
            }else if ((msplit[i].split("=")[0].substring(0,4)).equals("Note")){
                messObj[counter++]=msplit[i].split("=")[1].split(",")[0];
                messObj[counter++]=msplit[i].split("=")[2];
            }else{
                messObj[counter++]=msplit[i].split("=")[0];
                messObj[counter++]=msplit[i].split("=")[1];
            }
        }


        OSCMessage m = new OSCMessage("/chord", messObj);
        //********** do your work here ONLY **********
        messages[0] = m;
    }
}
