...
public class CodeTask7_3 implements BeadsChecker.BeadsCheckable {
    ...
    public void task(AudioContext ac, StringBuffer buf, Object... objects) {
        ...
        if (message.getName().equals("/panic")){
            noteStore.clear();
        } else {
            //noteoff
            if ((Integer)message.getArg(1) == 0){
                noteStore.remove(message.getArg(0));
            }else{
                noteStore.put((Integer)message.getArg(0),(Integer)message.getArg(1));
            }
        }
