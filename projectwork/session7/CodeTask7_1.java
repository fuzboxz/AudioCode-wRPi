...
public class CodeTask7_1 implements BeadsChecker.BeadsCheckable {
    ...
    public void task(AudioContext ac, StringBuffer buf, Object... objects) {
        ...
        if (message.getName().toLowerCase() == "chord" || message.getName().toLowerCase() == "/chord")
        {
            boolean wfFirst = true;
            // read waveform
            if (message.getArg(0).getClass()==String.class)
            {
                //waveform is the first argument
                buf.append("Waveform=" + message.getArg(0) + "\n");
            } else {
                //waveform is the last argument
                buf.append("Waveform=" + message.getArg(message.getArgCount()-1) + "\n");
                wfFirst = false;
            }

            if (wfFirst) {
                buf.append("Note1: MIDI=" + message.getArg(1) + ", gain=" + message.getArg(2) + "\n");
                buf.append("Note2: MIDI=" + message.getArg(3) + ", gain=" + message.getArg(4) + "\n");
                buf.append("Note3: MIDI=" + message.getArg(5) + ", gain=" + message.getArg(6) + "\n");

                for (int i = 7; i < message.getArgCount(); i= i+2) {
                    buf.append(message.getArg(i) + "=" + message.getArg(i+1)+"\n");
                }

            } else {
                buf.append("Note1: MIDI=" + message.getArg(0) + ", gain=" + message.getArg(1) + "\n");
                buf.append("Note2: MIDI=" + message.getArg(2) + ", gain=" + message.getArg(3) + "\n");
                buf.append("Note3: MIDI=" + message.getArg(4) + ", gain=" + message.getArg(5) + "\n");

                for (int i = 6; i < message.getArgCount()-1; i= i+2) {
                    buf.append(message.getArg(i) + "=" + message.getArg(i+1)+"\n");
                }
            }
        }else {
            buf.append("Message received with address \"" + message.getName() + "\"");
        }
        //********** do your work here ONLY **********
    }
}
