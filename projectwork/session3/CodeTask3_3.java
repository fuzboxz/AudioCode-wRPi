            ...
            protected void messageReceived(Bead bead) {
                if (c.getCount() % 4 == 0) {
                    ...
                    SamplePlayer sp = new SamplePlayer(ac, SampleManager.randomFromGroup("Guitar"));
                     ...
                     while (lastsample == sp.getSample().toString()) {
                            sp.setSample( SampleManager.randomFromGroup("Guitar"));

                     }
                     lastsample = sp.getSample().toString();

                    Envelope envPitch = new Envelope(ac, 1);

                    //every 10th note pitchbend
                    if (note == 9) {
                        envPitch.addSegment(rand.nextFloat() / 2, 300);
                        note = -1;
                    }
                    sp.setPitch(envPitch);
                    ...
                    gain.setGain(0.1f + ((rand.nextFloat() - 0.5f) / 10));
