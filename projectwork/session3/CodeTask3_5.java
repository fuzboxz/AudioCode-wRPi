...
public class CodeTask3_5 extends Application implements BeadsChecker.BeadsCheckable {
    ...
    public void task(AudioContext ac, StringBuffer stringBuffer, Object... objects) {
        ...
        Clock sh = new Clock(ac,lfoFreq);

        ZMap lfomapped = new ZMap(ac, 1);
        lfomapped.setRanges(-1f, 1f, freqLow, freqHigh);
        lfomapped.addInput(n);

        System.out.println("Freqlow: " + freqLow + " High: " + freqHigh);
        Glide filtFreq = new Glide(ac, 0);   //either replace the Glide object with a Function object, or use a Clock to control the Glide.

        sh.addMessageListener(new Bead() {
            @Override
            protected void messageReceived(Bead bead) {
                filtFreq.setValueImmediately(lfomapped.getValue());
                System.out.println("n " + n.getValue() + " freq: " + filtFreq.getValue() + " lfo: " + lfomapped.getValue());
            }
        });

        ac.out.addDependent(lfomapped);
        bf.setFrequency(filtFreq);
        bf.addInput(n);
        ac.out.addDependent(sh);
        ac.out.addInput(bf);
        //********** do your work here ONLY **********
    }
